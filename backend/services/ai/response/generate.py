...
def BotHandler(prompt: str, model: str, structure={"prompt_template": "Answer the following question: {}"}):
    ...
    if model.lower() not in supported_models:
        raise Exception(f"Unsupported model: '{model}'. Supported models are: {
    ...
    try:
        ...
        endpoint = os.getenv(f"AZURE_OPENAI_ENDPOINT_{
                             model.replace("-", "_")}")
        deployment = os.getenv(f"AZURE_OPENAI_DEPLOYMENT_{
                               model.replace("-", "_")}")
        api_version = os.getenv(f"AZURE_OPENAI_API_VERSION_{
                                model.replace("-", "_")}")

        # Check if environment variables for the model are available
        if not all([api_key, endpoint, deployment, api_version]):
            raise Exception(
                f"Missing environment variables for model: {model}")
        ...
        formatted_prompt = structure.get(
        ...
        raise Exception(
            "Invalid model name or environment variables are not set properly")
